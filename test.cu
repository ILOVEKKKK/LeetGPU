...
__global__ void matrix_mult(const float* A,const float* B,float* result,int M,int N,int K)
{
    ...
    for(int i = 0;i < (K+BLOCK_DIM-1)/BLOCK_DIM;++i)
    {
        ...
        {
            smemA[threadIdx.y][threadIdx.x] = A[tid_y*K+i*BLOCK_DIM+threadIdx.x];
        }
        else
        {
        ...
        {
            smemB[threadIdx.y][threadIdx.x] = B[(i*BLOCK_DIM+threadIdx.y)*N+tid_x];
        }
        else
        {
